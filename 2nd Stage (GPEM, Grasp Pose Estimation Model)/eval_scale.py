...
dump_folder='/home/dsp520/grasp/MSCG/logs/log_ssd_AFF_MSCG_seed_global_diff_R_High/dump_22_test_CD'
...
for scale in scales:
    ...
    res, ap = ge.eval_seen(dump_folder, proc=3)
    print("seen")
    ...
    res, ap = ge.eval_similar(dump_folder, proc=3)
    print("similar")
    print("AP",np.mean(res))
    res = res.transpose(3,0,1,2).reshape(6,-1)
    res = np.mean(res,axis=1)
    ...
    res, ap = ge.eval_novel(dump_folder, proc=3)
    print("novel")
    print("AP",np.mean(res))
    res = res.transpose(3,0,1,2).reshape(6,-1)
    res = np.mean(res,axis=1)
