...
import torch
from util.loss_utils import nn_distance, huber_loss

def get_loss(end_points):
    objectness_loss, end_points = compute_objectness_loss(end_points)  # objectness classification
    graspness_loss, end_points = compute_graspness_loss(end_points)  # regressions of point-wise graspable landscape
    view_loss, end_points = compute_view_graspness_loss(end_points)  # regressions of viewwise graspable landscape
    score_loss, end_points = compute_score_loss(end_points)  # regressions of grasp scores
    width_loss, end_points = compute_width_loss(end_points)  #  regressions of gripper widths
    loss = objectness_loss + 10 * graspness_loss + 100 * view_loss + 15 * score_loss + 10 * width_loss
    end_points['loss/overall_loss'] = loss
    return loss, end_points


def get_loss_stage(end_points, epoch):
    objectness_loss, end_points = compute_objectness_loss(end_points)
    graspness_loss, end_points = compute_graspness_loss(end_points)
    if epoch == 0:
        loss = objectness_loss + 10 * graspness_loss
        end_points['loss/overall_loss'] = loss
        return loss, end_points
...
# def compute_view_graspness_loss_SBL(end_points):
# class GraspNet_MSCG_context_high_SBL(nn.Module):  # In future work , modify SBL
