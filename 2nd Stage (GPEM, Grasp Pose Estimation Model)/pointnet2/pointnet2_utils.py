...
import sys
from models.attention import Point_Transformer
try:
    import builtins
except:
...
class CylinderQueryAndGroup_att(nn.Module):
    r"""
    Groups with a cylinder query of radius and height
    ...
    def __init__(self, radius, hmin, hmax, nsample, use_xyz=True, ret_grouped_xyz=False, normalize_xyz=False, 
                 rotate_xyz=True, sample_uniformly=False, ret_unique_cnt=False, split=True):
        super(CylinderQueryAndGroup_att, self).__init__()
        self.radius, self.nsample, self.hmin, self.hmax, = radius, nsample, hmin, hmax
        self.use_xyz = use_xyz
        self.split = split
        self.ret_grouped_xyz = ret_grouped_xyz
...
class OnlyGroup(nn.Module):
    def __init__(self,  nsample, use_xyz=True, ret_grouped_xyz=False, normalize_xyz=False, rotate_xyz=True, sample_uniformly=False, ret_unique_cnt=False):
        super(OnlyGroup, self).__init__()
    ...
    def forward(self, idx, xyz, new_xyz, rot, features=None):
        B, npoint, _ = new_xyz.size()
        if self.sample_uniformly:
            unique_cnt = torch.zeros((idx.shape[0], idx.shape[1]))
            for i_batch in range(idx.shape[0]):
